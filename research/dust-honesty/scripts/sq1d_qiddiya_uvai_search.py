"""
SQ1D Part A.7 — qiddiya_core UVAI all-months search + windowed re-rank.
...
def main():
    ...
    for y, m in iter_months(START_YM, END_YM):
        ...
        for pid, cp, dt_str in zip(ids, clouds, dates):
            tcol = (
                ...
                .filterDate(dt_str, next_day(dt_str))
                .filterBounds(geom)
            ...
            kept += 1
        print(f"  {y:04d}-{m:02d}: {len(ids)} scenes, kept {kept}")
    ...
    n_total = len(rows)
    n_lt_03 = sum(1 for r in rows if r["uvai_mean"] < 0.3)
    n_lt_0 = sum(1 for r in rows if r["uvai_mean"] < 0.0)
    print(f"\nTotal scene-dates with valid UVAI: {n_total}")
    print(f"Skipped (no overpass / null reduce): {skipped}")
    print(f"uvai_mean < 0.3: {n_lt_03}")
    print(f"uvai_mean < 0.0: {n_lt_0}")
    print(f"Wrote {n_total} rows → {OUT_CSV}")

    # Stop rules
    if n_total < 30:
        print("\nSTOP: fewer than 30 valid overpasses. Halt.")
        return
    rows_sorted = sorted(rows, key=lambda r: r["uvai_mean"])
    lowest = rows_sorted[0]["uvai_mean"]
    if lowest > 0.5:
        print(f"\nSTOP: lowest uvai_mean = {lowest:.4f} > 0.5. Halt — Qiddiya never had a clean column in this window.")
        # still print windows for record
    ...
    for label, start, end in WINDOWS:
        ...
        print(f"\n=== {label} ===")
        print(f"  scenes in window: {len(sub)}")
        print(f"  uvai_mean < 0.3:  {sum(1 for r in sub if r['uvai_mean'] < 0.3)}")
        print(f"  uvai_mean < 0.0:  {sum(1 for r in sub if r['uvai_mean'] < 0.0)}")
        print(
